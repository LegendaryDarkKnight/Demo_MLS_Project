...
* **Routes:** `/api/listings` (combo fetch), `/api/search` (Nominatim geo), `/api/search/reverse` (uncached), `/api/auth` (signup/signin/signout/me).
...
* **Data Flow:** Debounced `SearchBar` -> `handleSearch` -> bounds/location extraction -> React Query `useListings` -> client-side `useMemo` filtering (price, status, sort) -> Render.
